...
func check_flag_collision():
	if not detection_lever: return
	var areas = detection_lever.get_overlapping_areas()
	for area in areas:
		if area.name == "Flag" or area.is_in_group("flag"):
			var win_menu = get_tree().root.find_child("WinMenu", true, false)
			if win_menu:
				win_menu.show_win()

func update_animations(direction):
	if sprite.animation == "fist" and sprite.is_playing(): return
	if not is_on_floor():
		sprite.play("jump")
	elif direction != 0:
		sprite.play("walk")
...
func _on_water_entered(area: Area2D):
	if area.is_in_group("water"): die() 

func _on_lever_entered(area: Area2D):
	if area.is_in_group("lever"): lever_in_range = area

func _on_lever_exited(area: Area2D):
	if area == lever_in_range: lever_in_range = null
